...
def graph(n,gap,exp_name) :
	...
	for i in range(5,n+1,1) :
		...
		inc_errnx_results.append(get_results([err1.Process('P'+str(i+1),proc_lists['inc'][i]) for i in range(len(proc_lists['inc']))],err1.ERRN()))
		...
		dec_errnx_results.append(get_results([err1.Process('P'+str(i+1),proc_lists['dec'][i]) for i in range(len(proc_lists['dec']))],err1.ERRN()))
		...
		rand_errnx_results.append(get_results([err1.Process('P'+str(i+1),proc_lists['rand'][i]) for i in range(len(proc_lists['rand']))],err1.ERRN()))
